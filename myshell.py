...
if __name__ == "__main__":
    ...
    def print_usage():
        shell_print("Usage:")
        shell_print("  myshell.py                 # start interactive shell")
        shell_print("  myshell.py <script.sh>     # run commands from script file")
        shell_print("  myshell.py -c \"cmd...\"   # run a single command")
        shell_print("  myshell.py -h|--help       # show this help")
    ...
    if len(sys.argv) > 1:
        ...
        if arg1 == "-c":
            if len(sys.argv) < 3:
                shell_print("Error: -c requires a command string")
                print_usage()
                sys.exit(2)
            cmd = sys.argv[2]
            print(f"myshell$ {cmd}")
        ...
        if not os.path.exists(script_path):
            shell_print(f"Error: script file not found: {script_path}")
            print_usage()
            sys.exit(1)
        ...
        while i < len(lines):
            ...
            if line.startswith('if '):
                ...
                if (i + 1) < len(lines) and lines[i+1].strip() == 'then':
                    block = []
                    j = i+2
                ...
                values = for_parts[1].strip().rstrip(';').split() if len(for_parts) > 1 else []
                if (i + 1) < len(lines) and lines[i+1].strip() == 'do':
                    block = []
                    j = i+2
